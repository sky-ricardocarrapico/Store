...
class FlowStoreTest {
    ...
    @Test
    fun `GIVEN SoT has data and fetcher throws an error WHEN stream with fresh request THEN fallback to SoT`() = testScope.runBlockingTest {
        val exception = IllegalArgumentException("wow")
        val persister = InMemoryPersister<Int, String>().asFlowable()
        val pipeline = StoreBuilder.from(
                Fetcher.of {
        ...
        assertThat(pipeline.stream(StoreRequest.fresh(key = 3)))
                .emitsExactly(
                        Loading(
                                origin = ResponseOrigin.Fetcher
                        ),
